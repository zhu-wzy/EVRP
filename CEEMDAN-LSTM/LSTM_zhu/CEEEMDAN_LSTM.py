...
# road 1 61天
# 读取 CSV 文件
data = pd.read_csv(r"./IMF_1.csv")


# 提取第 5 列数据 (索引从 0 开始)
# 注意：假设第 5 列不包含标签，可以直接用 data.iloc[:, 4] 提取
# 如果有标签列，通常需要忽略第一列，所以需要根据实际文件结构调整索引
fst_column_data = data.iloc[:, 0][:3000]
plt.figure(figsize=(15, 6))  # 宽度为 12，高度为 6
plt.plot(fst_column_data)
plt.title('Line Plot of IMF')
plt.xlabel('Index (First 3000 Points)')
plt.ylabel('Value')
...
# 2 定义数据集
train_size = int(len(fst_column_data)*(3000/4462))
test_size = len(fst_column_data) - train_size
train, test = fst_column_data[:train_size], fst_column_data[train_size:]
...
if __name__ == "__main__":
    ...
    plt.plot(actual_values, label='fact', color='blue')
    plt.plot(test_outputs, label='predict', color='red')
    plt.title('fact_predict')

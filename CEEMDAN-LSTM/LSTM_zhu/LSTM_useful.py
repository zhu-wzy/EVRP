...
scalar = False  # 是否使用归一化
scalar_contain_labels = True  # 归一化过程是否包含目标值的历史数据
target_value = 'imf1'  # 需要预测的列名，可以在excel中查看
# 多步，单步标签
if output_length > 1:
...
if scalar_contain_labels and scalar:
    ...
    print('R2=%s'%R2)
    # 绘制图像的代码
    # 在这里添加代码将结果保存到Excel文件中

    # 创建一个DataFrame来保存预测结果和实际值
    results_df = pd.DataFrame({
    'Predicted': pre_array.flatten(),
    'Actual': test_labels.flatten()
    })
    # 保存到Excel文件
    results_df.to_excel('./forecast_results_IMF1.xlsx', index=False)
    print("Prediction results have been saved to 'forecast_results_IMF_1.xlsx'.")
